...
class Transaction extends Database
{
    ...
    public function save()
    {
        ...
        if (!$this->id) {
            ...
            $sql               = "INSERT INTO payme_transactions SET 
            ...
            $sql = "UPDATE payme_transactions SET
    ...
    public function find($params)
    {
        ...
        if (isset($params['id'])) {
            $sql        = "SELECT * FROM payme_transactions WHERE paycom_transaction_id = :pPaycomTxId";
            $sth        = $db->prepare($sql);
            $is_success = $sth->execute([':pPaycomTxId' => $params['id']]);
        } elseif (isset($params['account'], $params['account']['order_id'])) {
            // todo: Implement searching payme_transactions by given parameters and return the list of payme_transactions
            // search by order id active or completed transaction
            $sql        = "SELECT * FROM payme_transactions WHERE state IN (1, 2) AND order_id = :pOrderId";
            $sth        = $db->prepare($sql);
        ...
        // Search payme_transactions for a given period of time that specified in $params
    }

    /**
     * Gets list of payme_transactions for the given period including period boundaries.
     * @param int $from_date start of the period in timestamp.
     * @param int $to_date end of the period in timestamp.
     * @return array list of found payme_transactions converted into report format for send as a response.
     */
    public function report($from_date, $to_date)
    {
        ...
        // todo: Retrieve payme_transactions for the specified period from data store

        // Example implementation

        $db = self::db();

        $sql = "SELECT * FROM payme_transactions 
        ...
        // assume, here we have $rows variable that is populated with payme_transactions from data store
